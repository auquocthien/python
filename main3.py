from nckh.Vendor1 import Vendor1
...
def loop(T):
    ...
    # vị trí của  tcp trong arr1 bằng với tcp nhỏ nhất trong arr2
    vtmin = np.where(arr1 == min(arr2))
    vtmin = str(vtmin[0]) # chuyển từ tupe sang chuỗi
    vtmin = int(vtmin[1:-1]) # chuyển chuỗi đã cắt ngoặc vuông sang só
    arr1 = arr1[vtmin - 14: vtmin + 1]  # cắt chuỗi
...
start_time = time.time() # bắt đầu tính h
pr = Parameters1() # khai báo đối tượng thuộc 1 lớp
vd1 = Vendor1()
vd2 = Vendor1()
vd3 = Vendor1()
...
for i in range(9, 21): # vòng lăp chính
    str1 = loop(i)
    print(str1[: -3])
