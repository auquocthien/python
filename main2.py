...
def show(T, i, j, k, q1, q2, q3, q4, THC1, THC2, TOC, TPC2, TRC, TSC1, TSC2, TCp):
    str1 = ''
    arr = np.array([T, i, j, k, q1, q2, q3, q4, THC1, THC2, TOC, TPC2, TRC, TSC1, TSC2, TCp])
    for i in arr:
        str1 += str(i) + ": "
    return str1


def showinfile(T, i, j, k, q1, q2, q3, q4, THC1, THC2, TOC, TPC2, TRC, TSC1, TSC2, TCp):
        ...
        for j in range(1, T+1):
            for k in range(1, T+1):
                for l in range(1, T+1):
                    ...
                    q3 = round(vd3.q(T, k))
                    q4 = round(vd4.q(T, l))
                    THC1 = round((vd1.THC1(T, i) + vd2.THC1(T, j) + vd3.THC1(T, k) + vd4.THC1(T, l)) / T)
                    THC2 = round((vd1.THC2(T, i) + vd2.THC2(T, j) + vd3.THC2(T, k) + vd4.THC2(T, l)) / T)
                    TOC = round((vd1.TOC() + vd2.TOC() + vd3.TOC() + vd4.TOC()) / T)
                    TPC2 = round((vd1.TPC2(T, i) + vd2.TPC2(T, j) + vd3.TPC2(T, k) + vd4.TPC2(T, l)) / T)
                    TRC = round((vd1.TRC(T, i) + vd2.TRC(T, j) + vd3.TRC(T, k) + vd4.TRC(T, l)) / T)
                    TSC1 = round((pr.getA()) / T)
                    TSC2 = round((vd1.TSC2() + vd2.TSC2() + vd3.TSC2() + vd4.TSC2()) / T)
